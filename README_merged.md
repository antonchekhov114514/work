...
目标组织单元通过 `F_in` 获得各向同性体积增长或收缩，周围组织在有限元静力平衡中被动变形。当前基础材料模型为可压缩 Neo-Hookean，非线性系统使用 Newton 迭代和线搜索求解。
...
- 将体素模型转换为保留 `source_label`、`mesh_domain_tag` 和力学分组的四面体 FEM 网格；
- 支持规则粗化、边界感知的自适应局部加密和审计驱动三级加密；
- 按原始组织标签选择目标组织，例如：
  - `--target-label 1`：SAT；
...
当前代码同时保留四种标签。它们用途不同，不应互相替代：

| 字段 | 含义 | 主要用途 |
|---|---|---|
| `source_label` | 原始 0–73 组织标签，永久保留 | 组织追踪、目标选择、材料赋值、体积审计、分组织可视化 |
| `mesh_domain_tag` | 19 类二级网格/界面域 | 决定哪些组织可粗合并、哪些界面需要保护 |
| `mechanical_group_id` | 力学材料组 | 为 FEM 选择等效材料参数 |
| `cell_tags` / `region` | `SAT/SKIN/BONE/SOFT` 求解角色 | 快速选择增长区域和固定骨区域 |

19 个 `mesh_domain_tag` 为：
...
`mesh_domain_tag` 不是新的材料参数，也不会覆盖 `source_label`。例如：

- 只有 `source_label=1` 的 SAT 会被 `--target-label 1` 主动增减；
- 普通脂肪 `source_label=3` 和黄骨髓 `source_label=70` 保持被动；
- 心肌、CSF、神经、皮质骨和松质骨在二级网格域中仍被分别保护。
...
calibration_iterations[].solver_increments
    每轮校正实际使用的载荷增量数

calibration_iterations[].solver_retries
    每轮内部求解发生的自动重试次数
```

如果内部求解因线搜索失败、最大 Newton 次数或非下降方向而终止，校正器会按 `increment-retry-factor` 自动增加载荷增量后重试。上面的默认示例会依次尝试更细的增量划分。只有在所有单元 Jacobian 保持为正、且残差已进入 `stagnation-tolerance-factor × tolerance` 的严格邻域时，才允许把浮点精度导致的停滞视为可接受状态；明显未收敛或网格翻转不会被接受。
...
以下数字来自一份 `598 × 263 × 1645` 的 73 标签人体 MAT，仅用于说明不同网格策略的相对趋势，不能直接作为其他模型的验收阈值。

| 网格 | 四面体数 | 总组织体积 | SAT 误差 | Skin 误差 | Liver 误差 |
|---|---:|---:|---:|---:|---:|
| 原始体素 | — | 0.062866 m³ | — | — | — |
...
这些结果说明边界感知和审计驱动加密能明显改善主体组织体积，但极小脑内或气道结构仍可能低于当前 FEM 的有效机械分辨率。正式实验应根据自己的 `volume-audit` 决定网格，而不是照搬上述步长。
...
用途：恢复缺失标签并降低高误差组织的体积误差。`--audit-report` 可以重复提供，程序会累计历史报告中的缺失标签和高误差标签，避免全局重新四面体化后已恢复的小标签再次丢失。

只关注 SAT/皮肤界面时，可用重复的：

```bash
--refine-label 1 --refine-label 2
```

代替 `--refine-all-boundaries`，以降低点数和内存开销。
...
共享节点的 conforming 界面已经是绑定界面，不应重复建立接触。动态模式会在装配时更新最近主三角面、投影点和当前法向，但切线仍采用冻结投影/法向的准 Newton 近似，因此 `search-distance` 和 `penalty` 需要做敏感性分析。重网格后需要重建接触。
...
`tissues.vtm` 可以直接在 ParaView 中打开，每个组织是独立 block。生成不依赖 ParaView 的三视图：
...
分组织 bundle 会同时保留 `source_label`、`mesh_domain_tag`、机械组、颜色和透明度信息，因此既能按 73 个原始组织查看，也能按 19 个网格域汇总展示。
...
跨阶段保存 `accumulated_growth_J`、`elastic_history_F` 和 `material_reference_volume`。二分时子单元继承父单元状态；粗化时在同标签局部区域按参考材料体积投影。单元数量变化只改变离散分辨率，真实组织增减质量由累计增长 Jacobian 和密度决定。

接触乘子与摩擦历史尚未跨重网格传递，重网格后应重新建立接触。
...
`refine-result` 使用共形 edge-star 二分，并在整个四面体星域同步处理所选边，避免悬挂节点；界面邻居可能同步细分，但仍继承原 `source_label`。`coarsen-result` 只有在标签邻域、边界类型、link condition、局部逐标签体积漂移和正 Jacobian 均通过检查时才接受边塌缩。严格检查导致“没有可接受候选边”属于安全拒绝，不应通过关闭标签保护强行粗化。
...
```text
initial_mass_kg
growth_accounted_mass_kg
geometric_constant_density_mass_kg
```

其中 `growth_accounted_mass_kg` 按参考材料体积、密度和累计增长量计算；`geometric_constant_density_mass_kg` 只用于几何核查。质量报告必须结合 `volume-audit` 一起解释，弹性压缩 `J_elastic` 不代表组织质量消失。

### 10.9 材料与各向异性假设

项目中有两类材料数据：

1. **密度/电磁属性**：`mass_density_kg_per_m3`、`conductivity_s_per_m`、`relative_permittivity`、`frequency_hz`；
2. **FEM 力学属性**：`young` 和 `poisson`，以及可选纤维增强参数。

电磁属性不能直接当作超弹性参数。内置 Young 模量和泊松比属于研究型有效参数，骨、牙和肌腱等组织可能为数值稳定而软化，不能直接解释为临床材料常数。
...
普通脂肪/黄骨髓  被动脂肪样材料
皮肤             等效材料，可选规则估计的切向纤维
肌肉/肌腱         等效材料，可选 PCA/主轴方向纤维
骨               固定约束或高刚度材料
```

需要定量生理解释时，应使用文献、实验或 DTI 数据标定材料和纤维方向。

### 10.10 性能与规模建议

当前基础 Neo-Hookean 单元的能量/残差和局部切线已进行 NumPy 批量化，外层校正会复用上一轮位移作为初值，动态接触只对指定从面节点做 KD-tree 搜索。

约 80 万四面体的模型可以完成转换和体积审计，但纯 Python 全 Newton 刚度装配通常不适合直接正式求解。建议：

- 先在约 10 万至 15 万四面体上完成参数与流程验证；
- 使用审计决定局部加密区域，而不是全局盲目加密；
- 百万级网格和严格近不可压缩问题迁移到 PETSc/FEniCSx/MPI；
- 需要严格不可压缩压力场时使用混合 `u-p` 单元，而不是只依赖 `--bulk-modulus-ratio-cap`。
...
当前 `calibrate-growth` 支持自动细分失败的载荷增量。先确认当前代码已重新安装：
...
日志中的 `min(J)` 必须始终大于 0。若自动增加到 48 或 96 个增量后，残差仍远高于收敛阈值，应按顺序处理：

1. 将目标比例从 `1.20` 改为 `1.05`，确认小变形能否收敛；
2. 运行 `quality-report`，定位低质量或非正体积单元；
3. 增加 `--max-iterations`；
4. 降低过硬材料的 Young 模量；
5. 启用 `--bulk-modulus-ratio-cap 100`；
6. 暂时关闭接触，区分接触问题和材料/网格问题；
7. 检查固定骨区域是否过约束；
8. 使用 `solve-remesh` 将大变形拆成多个阶段。

不要仅通过持续放宽残差容差来掩盖明显未收敛或网格翻转。
...
1. `satmorph demo` 能运行；
2. 全部单元测试通过；
3. `source_label`、`mesh_domain_tag` 数量和逐标签体积没有异常漂移；
4. 100% 对照组位移接近 0；
...
- 动态接触使用准 Newton 近似，尚未包含严格几何一致切线、摩擦和跨重网格接触历史转移；
- 当前锁死控制不等同于严格混合 `u-p` 单元；
- 纯 Python Newton 装配不适合直接求解百万级四面体；
- 极小脑内、眼部和气道结构可能低于当前网格的有效机械分辨率；
