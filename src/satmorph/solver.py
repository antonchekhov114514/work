...
class SolverOptions:
    ...
    stagnation_tolerance_factor: float = 5.0
...
def morph_target_region(
    ...
    if options.stagnation_tolerance_factor < 1.0:
        raise ValueError("stagnation_tolerance_factor must be at least one")
        ...
        for iteration in range(options.max_iterations + 1):
            ...
            if not accepted:
                stagnation_threshold = (
                    options.stagnation_tolerance_factor * threshold
                )
                if (
                    norm <= stagnation_threshold
                    and float(jt.min()) > options.jacobian_floor
                ):
                    if options.verbose:
                        print(
                            "  line search stagnated near equilibrium; "
                            f"accepting |R|={norm:.6e} within "
                            f"{options.stagnation_tolerance_factor:.1f}x tolerance"
                        )
                    records.append(
                        IterationRecord(
                            increment,
                ...
                raise RuntimeError(
                    ...
                    "increase --increments, relax the nonlinear tolerance slightly, "
                    "or improve tetrahedral mesh quality"
                )
    ...
    return MorphResult(
        ...
        locking_control={
            ...
            "line_search_stagnation_tolerance_factor": (
                options.stagnation_tolerance_factor
            ),
